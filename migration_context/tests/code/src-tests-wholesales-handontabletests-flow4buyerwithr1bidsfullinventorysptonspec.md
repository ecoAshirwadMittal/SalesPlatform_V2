# Test Spec: Flow4_BuyerWithR1Bids_FullInventory_SptON.spec.ts

- **Path**: `src\tests\Wholesales\HandOnTableTests\Flow4_BuyerWithR1Bids_FullInventory_SptON.spec.ts`
- **Category**: Test Spec
- **Lines**: 280
- **Size**: 15,705 bytes
...
test.describe('Sales Can Setup R2 Criteria | Flow - Any Buyer with Round-1 Bids, Full Inventory, STB Flag ON @auctions-regression', () => {
    test('Sales Can Setup R2 Criteria and Start Auction R1', async () => {
        await base['auc_acc_Round2CriteriaPage'].selectRegularBuyerSettings('Any Buyer with Round-1 Bids', 'Full Inventory', 'Yes');
    ...
    test('R2 Live | Buyer Who Did Not Submit Bids in R1 Can NOT Access R2', async () => {
        await base['loginPage'].logout();
    ...
    test('R2 Live | Qualified Buyer Who Submitted Bids in R1 Can See Full Inventory with R1 Bids', async () => {  // AA156WHL       
