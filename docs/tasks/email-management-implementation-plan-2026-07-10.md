...
- [x] **Step 1: Failing tests (RTL)** — SMTP tab loads+saves (mock fetch), Templates tab lists+edits, Log tab lists+opens a detail; the detail view renders sanitized HTML (a `<script>` in the body is stripped). Delivered as 3 files (`emailAdmin.smtp/templates/log.test.tsx`, 13 cases), each rendering its tab component in isolation.
- [x] **Step 2: Run → FAIL** — `npm test -- emailAdmin` (13/13 failing against the snake_case stub).
- [x] **Step 3: Implement** — full snake→camel sweep (interfaces + read/write sides) against the actual Java records; also closed gaps the stub never had at all (no `templateKey`/`contentHtml` in the create form → every create 400'd; SMTP form had Username/Password fields that don't exist in `SmtpConfigView`/`SmtpConfigUpdate` per D2 → removed; `EmailStatus` is `PENDING/SENT/FAILED`, not the stub's `SENT/QUEUED/FAILED/ERROR` → fixed the filter + badge CSS; preview + send-test were entirely unwired → added). Split into `SmtpConfigTab.tsx` / `TemplatesTab.tsx` / `TemplateDetailEditor.tsx` / `EmailLogTab.tsx` + a shared `lib/adminEmailClient.ts` typed client (page.tsx is now a thin shell) to keep files under the repo's 800-line guideline. Replaced the regex `sanitizeEmailHtml` with `DOMPurify.sanitize(...)` (plain `dompurify`, not `isomorphic-dompurify` — verified SSR-safe: import doesn't throw without `window`, and the `.sanitize()` call sites are gated behind `useEffect`-populated state that's never present during SSR).
- [x] **Step 4: Run → PASS** — 13/13 green, 291/293 in the full frontend suite (2 pre-existing `apiFetch-guard` failures, unrelated to this task's files — verified). `npm run build`'s TypeScript gate is currently blocked by a **pre-existing, unrelated** type error in `partial-credit/[id]/AdminReviewClient.tsx` (verified via `git stash` that it reproduces on `HEAD` with this task's changes fully removed) plus a pre-existing `useSearchParams()` Suspense-boundary error on `/wholesale/partial-credit/new`. This task's own files have **zero** TypeScript errors (confirmed via a full `tsc --noEmit` sweep) and were confirmed to render correctly end-to-end in a real logged-in browser session (Playwright) — see task-10-report.md for full evidence. Both blockers need a fix from whoever owns that surface before a clean `npm run build` is possible on this branch.
- [x] **Step 5: Commit** — `feat(email): wire Email Admin UI to backend + real HTML sanitization (M-3)`.
