...
- **Status:** **FIXED 2026-07-12** — "Credit Requests" added to `BidderSidebar` between Auction and
  Buyer User Guide → `/wholesale/partial-credit` (reply-arrow icon). See ADR + shell-legacy-parity impl.
...
- **Status:** **FIXED 2026-07-12** — top-bar pill removed; new in-content `SwitchBuyerCodeCard`
  (label "Switch" green + "Buyer Code" dark, bordered card: buyer name small over CODE large). Chrome
  now shows the DIRECT logo + person name + green initials avatar (white initials). Identity-chip
  per-side masking remains a harness concern. See ADR + shell-legacy-parity impl.
