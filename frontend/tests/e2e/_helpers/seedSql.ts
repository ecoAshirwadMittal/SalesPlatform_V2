 ...
 * bare basename (`'wholesale-r1-active.sql'`).
 *
...
export function applyFixture(name: string): void {
  // Playwright invokes the test runner from the project root
  // (frontend/) so process.cwd() points at the directory containing
  // playwright.config.ts. Fixtures live under tests/_fixtures/.
  ...
  try {
    execFileSync(
      ...
      ['-h', host, '-U', user, '-d', db, '-v', 'ON_ERROR_STOP=1', '-f', sqlPath],
      {
        ...
        // psql output is small; 5MB is plenty for any error message.
