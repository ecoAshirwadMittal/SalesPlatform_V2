...
class R2InitStubListenerTest {

    private R2InitStubListener listener;
    private ListAppender<ILoggingEvent> appender;

    @BeforeEach
    void setUp() {
        listener = new R2InitStubListener();
        Logger l = (Logger) LoggerFactory.getLogger(R2InitStubListener.class);
    ...
    @Test
    void onRoundStarted_round2_logsStub() {
        listener.onRoundStarted(new RoundStartedEvent(22L, 2, 100L, 999L));
        assertThat(appender.list).hasSize(1);
        assertThat(appender.list.get(0).getFormattedMessage())
                .contains("[stub] R2 init")
                .contains("auctionId=100").contains("weekId=999").contains("roundId=22");
    }
    ...
    @Test
    void onRoundStarted_round3_isIgnored() {
