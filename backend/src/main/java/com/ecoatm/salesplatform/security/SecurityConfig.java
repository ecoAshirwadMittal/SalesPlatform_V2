    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(authz -> authz
                ...
                // Lane 1B — R3 bid report (read-only, SalesOps allowed)
                .requestMatchers("/api/v1/admin/bid-reports/**").hasAnyRole("Administrator", "SalesOps")
                // Lane 3A — bid data admin (DELETE further restricted to Administrator in @PreAuthorize)
                .requestMatchers("/api/v1/admin/bid-data/**").hasAnyRole("Administrator", "SalesOps")
                // Lane 3B — QBC admin (PATCH further restricted by @PreAuthorize if needed)
